    ...
    <body class="antialiased bg-gray-50 text-gray-900">
        ...
        <nav class="flex justify-between items-center px-8 py-4 shadow bg-ghana-gradient text-white">
            ...
            <div class="space-x-4">
                @if (Route::has('login'))
                    @auth
                        <a href="{{ url('/members') }}" class="px-4 py-2 rounded bg-white text-ghana-gradient font-semibold">
                            Dashboard
                        </a>
                    @else
        ...
        <section class="flex flex-col items-center justify-center text-center min-h-[80vh] px-6">
            <h2 class="text-4xl sm:text-6xl font-extrabold mb-6 text-ghana-gradient">
            ...
            <div class="space-x-4">
                <a href="{{ route('register') }}" class="px-6 py-3 bg-ghana-gradient text-white rounded-lg shadow-lg hover:opacity-90 transition">
                    Get Started
                </a>
                <a href="{{ route('login') }}" class="px-6 py-3 border-2 border-ghana-red text-ghana-gradient rounded-lg hover:bg-gray-100 transition">
                    Login
                </a>
            </div>
        </section>
