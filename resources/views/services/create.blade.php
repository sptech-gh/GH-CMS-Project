        ...
        <form action="{{ route('services.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" rows="4" required
                          class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-ghana-gradient @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex justify-between items-center">
                <a href="{{ route('services.index') }}" class="px-6 py-2 border-2 border-ghRed text-ghRed rounded-lg hover:bg-gray-100 hover:text-ghGold transition">
                    Cancel
                </a>
