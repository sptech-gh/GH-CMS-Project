<x-app-layout>
    <x-slot name="header">
        ...
        <p class="text-sm text-gray-500">Update the details for this member.</p>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-2xl overflow-hidden border border-gray-200">
            ...
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('members.update', $member->id) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="church_id" id="church_id"
                            ...
                            @foreach($churches as $church)
                                <option value="{{ $church->id }}" {{ old('church_id', $member->church_id) == $church->id ? 'selected' : '' }}>
                                    {{ $church->name }}
                                </option>
                            @endforeach
                        </select>
